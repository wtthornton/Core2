...
void draw_face_strip(M5Canvas& c, Screen screen, int w) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    const int x = 5 + i * tw;
    if (on) {
      c.fillSmoothRoundRect(x, 4, tw - 4, 22, 11, kTeal);
      c.fillSmoothRoundRect(x + 10, 24, tw - 24, 3, 1, kCyan);
    }
    c.setTextDatum(middle_center);
    c.setTextColor(on ? kWhite : kMute, on ? kTeal : kBg0);
    c.setTextSize(1);
    c.drawString(kFaceName[i], x + (tw - 4) / 2, 15);
  }
  c.setTextDatum(top_left);
}
...
void draw_hub(M5Canvas& c, const AppState& st, int w, int bat) {
  ...
  const int top = kStripH + 6;

  panel(c, 6, top, w - 12, 118, kPanel);
  draw_orb(c, 58, top + 58, 26, status_color(s), now);
  draw_iso_cube(c, w - 58, top + 56, 30, static_cast<uint8_t>(st.screen), g_cube_yaw);

  c.setTextColor(kWhite, kPanel);
  c.setTextSize(2);
  c.setCursor(100, top + 22);
  c.print(status_label(s));
  c.setTextSize(1);
  label_sm(c, 100, top + 48, s.ver[0] ? s.ver : "AF --", kInk, kPanel);
  char meta[40];
  snprintf(meta, sizeof(meta), "%d%%  %s", bat, s.link[0] ? s.link : "--");
  label_sm(c, 100, top + 64, meta, kMute, kPanel);
  if (st.mute_until_ms) {
    label_sm(c, 100, top + 80, "quiet on", kAmber, kPanel);
  }
  if (s.agent[0]) {
    label_sm(c, 100, top + 96, s.agent, kCyan, kPanel);
  }
  ...
  auto tile = [&](int i, const char* title, const char* val, uint16_t accent) {
    ...
    panel(c, x, y, tw, 46);
    c.fillSmoothRoundRect(x, y, 5, 46, 2, accent);
    label_sm(c, x + 12, y + 8, title, kMute, kPanel);
    c.setTextColor(kWhite, kPanel);
    c.setTextSize(2);
    c.setCursor(x + 12, y + 22);
    c.print(val);
    c.setTextSize(1);
  };
  tile(0, "INV", a, kCyan);
...
void draw_pulse(M5Canvas& c, const AppState& st, int w) {
  ...
  panel(c, 6, top, w - 12, 56);
  label_sm(c, 16, top + 8, "INVOCATIONS", kMute, kPanel);
  c.setTextColor(kWhite, kPanel);
  c.setTextSize(3);
  ...
  c.drawString(st.snap.agent[0] ? st.snap.agent : "agent", w - 18, top + 40);
  c.setTextDatum(top_left);
  draw_bars(c, 6, top + 64, w - 12, 102, st.series.inv, st.series.n_inv, kCyan);
}

void draw_heat(M5Canvas& c, const AppState& st, int w) {
  const FailState& f = st.fail;
  const int top = kStripH + 4;
  panel(c, 6, top, w / 2 - 9, 64);
  label_sm(c, 16, top + 8, "FAILS 24H", kMute, kPanel);
  c.setTextColor(kCoral, kPanel);
  c.setTextSize(3);
  c.setCursor(16, top + 28);
  c.printf("%ld", static_cast<long>(f.n24));
  c.setTextSize(1);

  panel(c, w / 2 + 3, top, w / 2 - 9, 64);
  label_sm(c, w / 2 + 13, top + 8, "ERROR %", kMute, kPanel);
  c.setTextColor(kAmber, kPanel);
  c.setTextSize(3);
  c.setCursor(w / 2 + 13, top + 28);
  if (st.snap.auth_ok && st.snap.reachable) {
  ...
  if (!f.have || f.n == 0) {
    ...
    c.setTextDatum(middle_center);
    c.setTextColor(kInk, kPanel);
    c.drawString("All quiet", w / 2, y + 40);
    c.setTextColor(kMute, kPanel);
    c.drawString("no recent failures", w / 2, y + 58);
    c.setTextDatum(top_left);
    return;
  }
...
void draw_trail(M5Canvas& c, const AppState& st, int w) {
  ...
  draw_sparkline(c, 6, top + 14, w - 12, 78, st.series.inv, st.series.n_inv, kCyan);
  label_sm(c, 10, top + 98, "7-DAY ERRORS", kAmber, kBg0);
  draw_bars(c, 6, top + 112, w - 12, 54, st.series.err, st.series.n_err, kAmber);
}
...
void draw_beam(M5Canvas& c, const AppState& st, int w) {
  ...
  const int top = kStripH + 4;
  panel(c, 6, top, w - 12, 108);
  draw_iso_cube(c, w / 2, top + 52, 42, static_cast<uint8_t>(st.screen), g_cube_yaw);

  panel(c, 6, top + 116, w - 12, 50);
  char host[48];
  snprintf(host, sizeof(host), "%.40s", n.af_url[0] ? n.af_url : "--");
  label_sm(c, 14, top + 124, host, kInk, kPanel);
  char line[48];
  snprintf(line, sizeof(line), "%s  ·  %s", n.project_slug[0] ? n.project_slug : "--",
           s.link[0] ? s.link : "--");
  label_sm(c, 14, top + 140, line, kMute, kPanel);
  c.setTextColor(s.auth_ok ? kMint : kAmber, kPanel);
  c.setCursor(14, top + 154);
  c.print(s.auth_ok ? "auth ok" : "need afp_ key");
}
